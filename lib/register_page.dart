    ...
    class _RegisterPageState extends State<RegisterPage> {
      ...
      Future<void> _pickImage(ImageSource source) async {
        try {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                  'Image picker plugin not available. Please check your configuration.'),
            ),
          );
        } catch (e) {
          developer.log('Error picking image: $e', name: 'RegisterPage');
      ...
      Future<void> _selectLocation() async {
        try {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content:
                  Text('Failed to select location. Please try again.'),
            ),
          );
        }
      }
      ...
      Future<void> _getLocationFromAddress() async {
        ...
        try {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Error getting location from address'),
            ),
          );
        }
      }
        ...
        try {
          ...
          if (_userImage != null) {
            final storageRef = FirebaseStorage.instance
                .ref()
                .child('user_images/${_phoneController.text}${_userImage!.path.split('.').last}');
    ...
    class MapSelectionPage extends StatefulWidget {
      ...
      @override
      _MapSelectionPageState createState() =>
          _MapSelectionPageState();
    }
    
    class _MapSelectionPageState
        extends State<MapSelectionPage> {
      LatLng? _selectedLocation;
      ...
      Widget build(BuildContext context) {
        return Scaffold(
          appBar:
              AppBar(title: const Text('Select Location')),
          body: FlutterMap(
                // Start of Selection
                ...
                FloatingActionButton(
                  ...
                  onPressed: () async {
                    try {
                      ...
                      developer.log('Error getting current location: $e', name: 'MapSelectionPage');
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
